...
import {
  ...
  ShieldCheck, Truck, Leaf, Menu, ArrowUpRight, MessageCircle, Star, ShoppingBag, LayoutGrid, Gift, CreditCard, MousePointer2, ChevronDown
} from "lucide-react";
    ...
    <div className="site">
      <style>{`
        ...
        *{box-sizing:border-box}html{scroll-behavior:smooth}body{margin:0;background:${C.cream};color:${C.ink};font-family:Manrope,sans-serif}.sg{font-family:'Space Grotesk',sans-serif}button,input,textarea{font:inherit}button{cursor:pointer}.site{min-height:100vh;overflow:hidden}
        .nav{position:sticky;top:0;z-index:40;background:rgba(3,15,27,.7);backdrop-filter:blur(10px);border-bottom:1px solid rgba(164,238,238,.08);color:#fff;transition:background .3s ease,backdrop-filter .3s ease,border-color .3s ease,box-shadow .3s ease}.nav.scrolled{background:rgba(3,15,27,.93);backdrop-filter:blur(20px);border-bottom-color:rgba(164,238,238,.16);box-shadow:0 12px 34px rgba(0,0,0,.22)}.nav-inner{max-width:1620px;margin:auto;padding:11px 24px;display:flex;align-items:center;gap:30px}.brand{display:flex;align-items:center;gap:11px;min-width:245px}.brand-mark{width:43px;height:43px;border-radius:13px;border:1px solid rgba(18,215,230,.6);display:grid;place-items:center;background:linear-gradient(145deg,rgba(18,215,230,.17),rgba(122,92,255,.08));box-shadow:0 0 24px rgba(18,215,230,.15);transition:box-shadow .3s ease,transform .3s ease}.brand:hover .brand-mark{box-shadow:0 0 34px rgba(18,215,230,.45),0 0 14px rgba(177,74,255,.35);transform:scale(1.06)}.brand-name{font:700 22px 'Space Grotesk';letter-spacing:-.035em}.brand-sub{font-size:10px;color:#8fb2b4;letter-spacing:.14em;text-transform:uppercase;margin-top:2px}.navlinks{display:flex;align-items:center;gap:7px;flex:1;justify-content:center}.navlinks button{border:0;background:transparent;color:#c9dddd;padding:12px 13px;border-radius:10px;font-weight:600;font-size:14px;transition:.2s;position:relative}.navlinks button:hover,.navlinks button.active{color:#fff;background:rgba(255,255,255,.05)}.navlinks button:after{content:"";position:absolute;left:13px;right:13px;bottom:6px;height:2px;background:linear-gradient(90deg,${C.aqua},${C.purple2});border-radius:2px;transform:scaleX(0);transform-origin:center;transition:transform .3s ease}.navlinks button:hover:after{transform:scaleX(.5)}.navlinks button.active:after{transform:scaleX(1)}.cart-btn{border:1px solid rgba(18,215,230,.55);background:linear-gradient(135deg,#24b7cf,#2d91c5);color:#041321;border-radius:13px;padding:11px 16px;font-weight:800;display:flex;align-items:center;gap:8px;box-shadow:0 8px 28px rgba(18,215,230,.17);position:relative}.cart-badge{position:absolute;top:-7px;right:-7px;min-width:19px;height:19px;padding:0 4px;border-radius:999px;background:${C.pink};color:#fff;font-size:10.5px;font-weight:800;display:grid;place-items:center;border:2px solid ${C.navy}}
        .nav-search-btn{border:1px solid rgba(255,255,255,.14);background:rgba(255,255,255,.04);color:#cfeaea;width:40px;height:40px;border-radius:11px;display:grid;place-items:center;flex:none;transition:.2s}.nav-search-btn:hover{background:rgba(18,215,230,.12);color:#fff}
        .nav-search{display:flex;align-items:center;gap:0;overflow:hidden;width:0;opacity:0;color:#9bb8ba;transition:width .3s ease,opacity .2s ease}.nav-search.open{width:210px;opacity:1;gap:8px}.nav-search input{background:transparent;border:0;outline:0;color:#fff;font-size:13px;width:100%}.nav-search input::placeholder{color:#8ba3a5}
        ...
        .hero-card:nth-child(1) .mini-image .product-art:after{background-image:url('https://images.unsplash.com/photo-1544551763-46a013bb70d5?auto=format&fit=crop&w=800&q=85')}
        .hero-card:nth-child(2) .mini-image .product-art:after{background-image:url('https://images.unsplash.com/photo-1559454403-b8fb88521f11?auto=format&fit=crop&w=800&q=85')}
        .hero-card:nth-child(3) .mini-image .product-art:after{background-image:url('https://images.unsplash.com/photo-1485955900006-10f4d324d411?auto=format&fit=crop&w=800&q=85')}
        .hero-card:nth-child(4) .mini-image .product-art:after{background-image:url('https://images.unsplash.com/photo-1586023492125-27b2c045efd7?auto=format&fit=crop&w=800&q=85')}
      ...
      <header className={`nav ${scrolled ? "scrolled" : ""}`}>
        <div className="nav-inner">
          ...
          <div className={`nav-search ${navSearchOpen ? "open" : ""}`}><Search size={15}/><input placeholder="Search products…" value={query} onChange={e => setQuery(e.target.value)} onKeyDown={e => {if(e.key==="Enter"){scrollTo("shop");setNavSearchOpen(false)}}} autoFocus={navSearchOpen}/></div>
      ...
      <footer><div className="footer-inner"><div><div className="footer-brand">Aqua Dreamland</div><p>Aquariums, décor, gifts and little things that make your space feel alive.</p></div><div className="footer-links"><span><MapPin size={14}/> V.P.O. Neerpur (Narnaul)</span><span><Phone size={14}/> +91 7015280545</span><span>© 2026 Aqua Dreamland</span></div></div></footer>
