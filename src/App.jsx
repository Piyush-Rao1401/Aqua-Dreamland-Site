...
`}</style>
      ...
      {/* ---------- Hero ---------- */}
      <section id="home" style={{position:"relative",overflow:"hidden",minHeight:"calc(100vh - 69px)",color:C.white,isolation:"isolate",backgroundImage:`linear-gradient(90deg,rgba(1,9,22,.78) 0%,rgba(2,13,32,.60) 34%,rgba(2,11,29,.18) 62%,rgba(2,7,18,.28) 100%),url("/WhatsApp%20Image%202026-09-10%20at%208.01.18%20PM.jpeg")`,backgroundSize:"cover",backgroundPosition:"center center",padding:"54px 22px 72px"}}>
        ...
        <div className="heroMain" style={{maxWidth:1550,margin:"0 auto",display:"grid",gridTemplateColumns:"1.02fr .98fr",gap:34,alignItems:"center",position:"relative",zIndex:2}}>
          <div>
            ...
            <h1 className="sg heroTitle" style={{fontSize:"clamp(40px,3.8vw,62px)",lineHeight:1.02,margin:0,fontWeight:700,letterSpacing:"-.055em",maxWidth:760}}>
          ...
          <div className="fishSpace" style={{position:"relative",minHeight:500}}>
            ...
            <div className="heroGrid" style={{position:"relative",zIndex:2,display:"grid",gridTemplateColumns:"1fr 1fr",gap:14,paddingTop:34}}>
              {["aquariums","soft-toys","pots","resin"].map((c,idx)=>{
                ...
                return <button key={c} className="heroCard glass" onClick={()=>{setActiveCat(c);document.getElementById("shop")?.scrollIntoView({behavior:"smooth"})}} style={{textAlign:"left",position:"relative",background:"rgba(4,17,43,.60)",border:`1px solid ${colors[idx]}88`,borderRadius:18,padding:18,color:"#fff",boxShadow:"0 16px 42px rgba(0,0,0,.32)",minHeight:190}}>
                  <img src={CATEGORY_IMAGES[c]} alt="" loading="lazy" style={{position:"absolute",right:0,top:0,bottom:0,width:"56%",height:"100%",objectFit:"cover",objectPosition:"center",opacity:.9,filter:"saturate(1.08) contrast(1.03)"}}/>
                  <div style={{position:"absolute",inset:0,background:"linear-gradient(90deg,rgba(3,14,35,1) 0%,rgba(3,14,35,.94) 34%,rgba(3,14,35,.62) 58%,rgba(3,14,35,.08) 100%)"}}/>
