...
const CAT_VISUALS = {
  aquariums: {
    image: "https://images.unsplash.com/photo-1781695125669-b21426a2bdf3?auto=format&fit=crop&q=85&w=1200",
    tint: "rgba(31,214,255,.18)",
  },
  "soft-toys": {
    image: "https://images.unsplash.com/photo-1765913186506-a4f1ca060ced?auto=format&fit=crop&q=85&w=1200",
    tint: "rgba(217,87,255,.22)",
  },
  pots: {
    image: "https://images.unsplash.com/photo-1769016801417-958a7241d152?auto=format&fit=crop&q=85&w=1200",
    tint: "rgba(68,229,183,.16)",
  },
  resin: {
    image: "https://images.unsplash.com/photo-1618005198919-d3d4b5a92ead?auto=format&fit=crop&q=85&w=1200",
    tint: "rgba(195,107,255,.22)",
  },
};
...
.heroCard{position:relative;overflow:hidden;transition:.25s;min-height:174px}
...
.productArt:before{content:"";position:absolute;inset:0;background:linear-gradient(135deg,rgba(255,255,255,.04),transparent 40%,rgba(0,0,0,.22));pointer-events:none}
...
@media(max-width:900px){.navLinks{display:none!important}.heroGrid{grid-template-columns:1fr 1fr!important}.heroMain{grid-template-columns:1fr!important}.fishSpace{min-height:280px!important;order:-1}.heroTitle{font-size:clamp(38px,7.5vw,58px)!important}}
@media(max-width:620px){.heroGrid{grid-template-columns:1fr!important}.heroTitle{font-size:clamp(34px,11vw,50px)!important}.statsPill{display:grid!important;grid-template-columns:1fr!important}.statDivider{display:none}.chatBtn{right:16px;bottom:16px}.navWrap{padding-left:14px!important;padding-right:14px!important}}
`}</style>
      ...
      <header style={{ position:"sticky", top:0, zIndex:30, background:"rgba(2,9,20,.72)", backdropFilter:"blur(18px)", borderBottom:"1px solid rgba(93,205,255,.13)", color:C.white }}>
        <div className="navWrap" style={{ maxWidth:1280, margin:"0 auto", padding:"11px 22px", display:"flex", alignItems:"center", justifyContent:"space-between", gap:20 }}>
          <a href="#home" style={{display:"flex",alignItems:"center",gap:11,textDecoration:"none",color:"#fff"}}>
            ...
            <div>
              <div className="sg" style={{fontSize:21,fontWeight:700,letterSpacing:"-.025em"}}>Aqua Dreamland</div>
      ...
      {/* ---------- Hero ---------- */}
      <section id="home" style={{position:"relative",overflow:"hidden",minHeight:680,color:C.white,isolation:"isolate",backgroundImage:`linear-gradient(90deg,rgba(1,8,20,.96) 0%,rgba(2,12,29,.80) 30%,rgba(2,10,27,.28) 58%,rgba(2,7,18,.18) 100%),url("/betta-fish.jpg")`,backgroundSize:"cover",backgroundPosition:"center center",padding:"46px 22px 82px"}}>
        ...
        <div className="heroMain" style={{maxWidth:1280,margin:"0 auto",display:"grid",gridTemplateColumns:"1fr 1fr",gap:34,alignItems:"center",position:"relative",zIndex:2}}>
          <div>
            ...
            <h1 className="sg heroTitle" style={{fontSize:"clamp(40px,4.35vw,62px)",lineHeight:1.02,margin:0,fontWeight:700,letterSpacing:"-.055em",maxWidth:680}}>
              Everything for your<br/>
              <span className="gradientText" style={{fontStyle:"italic"}}>tank and everything</span><br/>
              around it.
            </h1>
            <p style={{marginTop:22,fontSize:16.5,lineHeight:1.72,color:"#C9D9E8",maxWidth:600}}>
          ...
          <div className="fishSpace" style={{position:"relative",minHeight:520}}>
            ...
            <div className="heroGrid" style={{position:"relative",zIndex:2,display:"grid",gridTemplateColumns:"1fr 1fr",gap:14,paddingTop:34}}>
              {["aquariums","soft-toys","pots","resin"].map((c,idx)=>{
                ...
                return <button key={c} className="heroCard glass" onClick={()=>{setActiveCat(c);document.getElementById("shop")?.scrollIntoView({behavior:"smooth"})}} style={{textAlign:"left",position:"relative",background:"rgba(4,14,32,.70)",border:`1px solid ${colors[idx]}66`,borderRadius:20,padding:18,color:"#fff",boxShadow:"0 16px 48px rgba(0,0,0,.30)",minHeight:184}}>
                  <div style={{position:"absolute",inset:0,borderRadius:20,overflow:"hidden",pointerEvents:"none"}}>
                    <img src={CAT_VISUALS[c].image} alt="" loading="lazy" style={{position:"absolute",right:0,bottom:0,width:"72%",height:"100%",objectFit:"cover",opacity:.82,mixBlendMode:"screen"}} />
                    <div style={{position:"absolute",inset:0,background:`linear-gradient(90deg,rgba(3,13,31,.98) 0%,rgba(3,13,31,.88) 38%,rgba(3,13,31,.34) 72%,rgba(3,13,31,.12) 100%),linear-gradient(180deg,transparent 45%,${CAT_VISUALS[c].tint})`}}/>
                  </div>
                  <div style={{position:"relative",zIndex:2}}>
                    <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
                      <div className="iconBubble" style={{borderColor:colors[idx]+"66"}}><Icon size={21} color={colors[idx]}/></div>
                      <div style={{width:34,height:34,borderRadius:50,display:"flex",alignItems:"center",justifyContent:"center",background:"rgba(255,255,255,.08)",border:`1px solid ${colors[idx]}66`}}><ChevronRight size={18} color="#fff"/></div>
                    </div>
                    <div className="sg" style={{fontSize:18,fontWeight:800,marginTop:15,textShadow:"0 2px 18px rgba(0,0,0,.55)"}}>{c==="soft-toys"?"Soft Toys":c==="resin"?"Resin Décor":c[0].toUpperCase()+c.slice(1)}</div>
                    <div style={{fontSize:12.2,lineHeight:1.5,color:"#D1DFEC",marginTop:7,maxWidth:220,textShadow:"0 2px 12px rgba(0,0,0,.7)"}}>{CAT_META[c].blurb}</div>
                  </div>
                </button>
              })}
            </div>
        ...
        <div style={{ display: "flex", flexWrap: "wrap", gap: 14, alignItems: "center", justifyContent: "space-between", marginBottom: 26 }}>
          ...
          <div style={{ position: "relative" }}>
            ...
            <input
              ...
              style={{ border: `1px solid ${C.sandDeep}`, borderRadius: 8, padding: "9px 12px 9px 32px", fontSize: 14, width: 210, background: "rgba(255,255,255,.05)", color:"#fff" }}
            />
          </div>
        </div>
              ...
              <div style={{ padding: 20, borderTop: "1px solid rgba(140,200,240,.15)" }}>
                ...
                {checkoutStep === "cart" ? (
                  <button
                    ...
                    className="addbtn"
                    style={{
                      width: "100%", background: cartItems.length ? "linear-gradient(135deg,#20D6FF,#7564FF)" : "#26364A", color: "#fff", border: "none",
                  ...
                  <button
                    ...
                    form="checkout-form"
                    className="addbtn"
                    style={{ width: "100%", background: "linear-gradient(135deg,#20D6FF,#7564FF)", color: "#fff", border: "none", borderRadius: 8, padding: "13px", fontWeight: 600, fontSize: 15 }}
                  >
                    Place Order
                  </button>
                )}
