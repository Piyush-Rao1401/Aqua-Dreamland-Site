...
@media(max-width:900px){.navLinks{display:none!important}.heroGrid{grid-template-columns:1fr 1fr!important}.heroMain{grid-template-columns:1fr!important}.fishSpace{min-height:280px!important;order:-1}.heroTitle{font-size:clamp(42px,9vw,70px)!important}}
@media(max-width:620px){.heroGrid{grid-template-columns:1fr!important}.heroTitle{font-size:clamp(38px,13vw,58px)!important}.statsPill{display:grid!important;grid-template-columns:1fr!important}.statDivider{display:none}.chatBtn{right:16px;bottom:16px}.navWrap{padding-left:14px!important;padding-right:14px!important}}
`}</style>
      ...
      {/* ---------- Hero ---------- */}
      <section id="home" style={{position:"relative",overflow:"hidden",minHeight:710,color:C.white,isolation:"isolate",backgroundImage:`linear-gradient(90deg,rgba(1,9,22,.96) 0%,rgba(2,13,32,.88) 30%,rgba(2,11,29,.48) 62%,rgba(2,7,18,.48) 100%),url("/betta-fish.jpg")`,backgroundSize:"cover",backgroundPosition:"center 45%",padding:"58px 22px 86px"}}>
        ...
        <div className="heroMain" style={{maxWidth:1280,margin:"0 auto",display:"grid",gridTemplateColumns:"1.05fr .95fr",gap:34,alignItems:"center",position:"relative",zIndex:2}}>
          <div>
            ...
            <h1 className="sg heroTitle" style={{fontSize:"clamp(48px,6.1vw,82px)",lineHeight:1.02,margin:0,fontWeight:700,letterSpacing:"-.055em",maxWidth:780}}>
          ...
          <div className="fishSpace" style={{position:"relative",minHeight:520}}>
            ...
            <div className="heroGrid" style={{position:"relative",zIndex:2,display:"grid",gridTemplateColumns:"1fr 1fr",gap:14,paddingTop:34}}>
              {["aquariums","soft-toys","pots","resin"].map((c,idx)=>{
                ...
                return <button key={c} className="heroCard glass" onClick={()=>{setActiveCat(c);document.getElementById("shop")?.scrollIntoView({behavior:"smooth"})}} style={{textAlign:"left",background:"linear-gradient(145deg,rgba(6,28,61,.72),rgba(4,13,32,.56))",border:`1px solid ${colors[idx]}55`,borderRadius:18,padding:18,color:"#fff",boxShadow:"0 14px 45px rgba(0,0,0,.22)"}}>
                  <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}><div className="iconBubble" style={{borderColor:colors[idx]+"66"}}><Icon size={21} color={colors[idx]}/></div><div style={{width:34,height:34,borderRadius:50,display:"flex",alignItems:"center",justifyContent:"center",background:"rgba(255,255,255,.07)",border:`1px solid ${colors[idx]}66`}}><ChevronRight size={18} color="#fff"/></div></div>
                  <div className="sg" style={{fontSize:18,fontWeight:800,marginTop:15}}>{c==="soft-toys"?"Soft Toys":c==="resin"?"Resin Décor":c[0].toUpperCase()+c.slice(1)}</div>
                  <div style={{fontSize:12.5,lineHeight:1.55,color:"#B4C8DA",marginTop:7,maxWidth:230}}>{CAT_META[c].blurb}</div>
                  <div style={{position:"absolute",right:-30,bottom:-40,width:150,height:150,borderRadius:"50%",background:`radial-gradient(circle,${colors[idx]}35,transparent 70%)`,filter:"blur(8px)"}}/>
                </button>
              })}
            </div>
