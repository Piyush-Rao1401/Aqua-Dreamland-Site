...
const PRODUCTS = [
  { id: "aq-1e", cat: "aquariums", name: "Aquarium 1 ft — Empty", price: 500, note: "Glass tank only", badge: "Starter", stock: "in-stock" },
  { id: "aq-1c", cat: "aquariums", name: "Aquarium 1 ft — Complete Set", price: 1000, note: "5mm glass · full setup", badge: "Popular", stock: "made-to-order" },
  { id: "aq-15e", cat: "aquariums", name: "Aquarium 1.5 ft — Empty", price: 900, note: "Glass tank only", stock: "in-stock" },
  { id: "aq-15c", cat: "aquariums", name: "Aquarium 1.5 ft — Complete Set", price: 2500, priceMax: 3000, note: "5mm glass · full setup", stock: "made-to-order" },
  { id: "aq-2e", cat: "aquariums", name: "Aquarium 2 ft — Empty", price: 1100, note: "Glass tank only", stock: "in-stock" },
  { id: "aq-2c", cat: "aquariums", name: "Aquarium 2 ft — Complete Set", price: 4000, priceFrom: true, note: "5mm glass · full setup", badge: "Popular", stock: "made-to-order" },
  { id: "aq-3e", cat: "aquariums", name: "Aquarium 3 ft — Empty", price: 2500, note: "8mm glass tank only", stock: "in-stock" },
  { id: "aq-3c", cat: "aquariums", name: "Aquarium 3 ft — Complete Set", price: 6000, priceFrom: true, note: "8mm glass · full setup", stock: "made-to-order" },
  { id: "st-1", cat: "soft-toys", name: "Teddy Bear — Small", price: 150, note: "Starting price · size varies", badge: "From ₹150", stock: "in-stock" },
  { id: "st-2", cat: "soft-toys", name: "Teddy Bear — Medium", price: 280, note: "Soft plush · medium", stock: "in-stock" },
  { id: "st-3", cat: "soft-toys", name: "Plush Bunny", price: 200, note: "Soft plush · assorted", stock: "in-stock" },
  { id: "st-4", cat: "soft-toys", name: "Cartoon Character Plush", price: 350, note: "Assorted designs", stock: "in-stock" },
  { id: "pt-1", cat: "pots", name: "Plastic Pot — Small", price: 15, note: "Starting price · varies by size", stock: "in-stock" },
  { id: "pt-2", cat: "pots", name: "Plastic Pot — Medium", price: 40, note: "Lightweight · indoor/outdoor", stock: "in-stock" },
  { id: "pt-3", cat: "pots", name: "Ceramic Pot — Small", price: 120, note: "Decorative ceramic", stock: "in-stock" },
  { id: "pt-4", cat: "pots", name: "Decorative Pot — Large", price: 250, note: "Statement planter", stock: "in-stock" },
  { id: "rs-1", cat: "resin", name: "Resin Photo Frame", price: 300, note: "Handmade · starting price", stock: "in-stock" },
  { id: "rs-2", cat: "resin", name: "Resin Keychain", price: 350, note: "Handmade · assorted", stock: "in-stock" },
  { id: "rs-3", cat: "resin", name: "Resin Coaster Set", price: 450, note: "Handmade · set", stock: "in-stock" },
  { id: "rs-4", cat: "resin", name: "Resin Table Decor", price: 600, note: "Handmade · statement piece", stock: "in-stock" },
];
...
export default function App() {
  ...
  const shopStatus = useMemo(() => {
    ...
    const openMin = isSunday ? 8 * 60 : 7 * 60;
    const closeMin = isSunday ? 20 * 60 : 21 * 60;
    const isOpen = mins >= openMin && mins < closeMin;
    return {
      isOpen,
      today: isSunday ? "8:00 AM – 8:00 PM" : "7:00 AM – 9:00 PM",
    };
  }, [now]);
    ...
    <div className="site">
      <style>{`
        ...
        *{box-sizing:border-box}html{scroll-behavior:smooth}body{margin:0;background:${C.cream};color:${C.ink};font-family:Manrope,sans-serif}.sg{font-family:'Space Grotesk',sans-serif}button,input,textarea{font:inherit}button{cursor:pointer}.site{min-height:100vh;overflow:hidden}
        ...
        .hours-badge{display:inline-flex;align-items:center;gap:8px;border-radius:999px;padding:6px 13px;font-weight:800;font-size:11.5px;border:1px solid rgba(255,255,255,.12);background:rgba(255,255,255,.04);color:#cfe6e5;margin-bottom:14px}
        .hours-badge .status-dot{width:8px;height:8px;border-radius:50%;flex:none}
        .hours-badge.open .status-dot{background:#22C58B;box-shadow:0 0 10px #22C58B}
        .hours-badge.closed .status-dot{background:#FF4F6D;box-shadow:0 0 10px #FF4F6D}
        .hours-badge.open{color:#8ff0c9;border-color:rgba(34,197,139,.35);background:rgba(34,197,139,.08)}
        .hours-badge.closed{color:#ffb3bf;border-color:rgba(255,79,109,.3);background:rgba(255,79,109,.07)}
        .hours-note{font-size:11px;color:#5f767a;margin-top:2px}
        ...
        .trust-band{background:#051019;border-top:1px solid rgba(18,215,230,.12);border-bottom:1px solid rgba(18,215,230,.12);padding:16px 24px;display:flex;justify-content:center;align-items:center;gap:44px;flex-wrap:wrap}.trust-band span{display:flex;align-items:center;gap:8px;color:#cfe6e5;font-size:13px;font-weight:700}.trust-band svg{color:${C.aqua};animation:icon-bob 2.6s ease-in-out infinite}.trust-band span:nth-child(2) svg{animation-delay:.3s}.trust-band span:nth-child(3) svg{animation-delay:.6s}
        .pay-badges{display:flex;align-items:center;gap:9px}
        .pay-logo{height:26px;width:auto;border-radius:6px;display:block}
        @keyframes icon-bob{0%,100%{transform:translateY(0)}50%{transform:translateY(-3px)}}
        @media(max-width:700px){.trust-band{gap:18px;padding:14px 16px}.trust-band span{font-size:11px}.pay-logo{height:22px}.footer-map{padding:0 18px 32px}.footer-map iframe{height:220px}}
        ...
        .product-grid.reveal,.category-grid.reveal,.care-grid.reveal,.testi-grid.reveal{opacity:1;transform:none;transition:none}
        .product-grid.reveal>*,.category-grid.reveal>*,.care-grid.reveal>*,.testi-grid.reveal>*{opacity:0;transform:translateY(22px);transition:opacity .6s ease,transform .6s ease}
        .product-grid.reveal.in>*,.category-grid.reveal.in>*,.care-grid.reveal.in>*,.testi-grid.reveal.in>*{opacity:1;transform:translateY(0)}
        .product-grid.reveal>*:nth-child(2),.category-grid.reveal>*:nth-child(2),.care-grid.reveal>*:nth-child(2),.testi-grid.reveal>*:nth-child(2){transition-delay:70ms}
        .product-grid.reveal>*:nth-child(3),.category-grid.reveal>*:nth-child(3),.care-grid.reveal>*:nth-child(3),.testi-grid.reveal>*:nth-child(3){transition-delay:140ms}
        .product-grid.reveal>*:nth-child(4),.category-grid.reveal>*:nth-child(4),.care-grid.reveal>*:nth-child(4){transition-delay:210ms}
        .swim-fish{position:absolute;top:18%;left:-60px;z-index:1;color:rgba(18,215,230,.4);animation:swim-across 26s linear infinite;pointer-events:none}
        .swim-fish svg{animation:fin-wiggle 1.1s ease-in-out infinite}
        .gallery .swim-fish{top:auto;bottom:14%;color:rgba(177,74,255,.32);animation-duration:32s;animation-delay:-9s}
        @keyframes swim-across{0%{transform:translateX(0) translateY(0) scaleX(1)}48%{transform:translateX(115vw) translateY(-18px) scaleX(1)}50%{transform:translateX(115vw) translateY(-18px) scaleX(-1)}98%{transform:translateX(0) translateY(14px) scaleX(-1)}100%{transform:translateX(0) translateY(0) scaleX(1)}}
        @keyframes fin-wiggle{0%,100%{transform:rotate(0deg)}50%{transform:rotate(8deg)}}
        @media(max-width:700px){.swim-fish{display:none}}
        .overlay{animation:overlay-in .25s ease}
        @keyframes overlay-in{0%{opacity:0}100%{opacity:1}}
        .drawer{animation:drawer-in .4s cubic-bezier(.22,1,.36,1)}
        @keyframes drawer-in{0%{transform:translateX(100%)}100%{transform:translateX(0)}}
        .legal-modal{animation:modal-in .3s cubic-bezier(.22,1,.36,1)}
        @keyframes modal-in{0%{opacity:0;transform:translateY(14px) scale(.98)}100%{opacity:1;transform:translateY(0) scale(1)}}
        @media(max-width:700px){.nav-inner{padding:9px 15px}.brand{min-width:0}.brand-name{font-size:18px}.brand-sub{font-size:8.5px}.cart-btn{padding:9px 11px}.hero{min-height:auto}.hero:before{background:linear-gradient(180deg,rgba(2,11,22,.35) 0%,rgba(3,17,32,.15) 40%,rgba(2,12,25,.1) 65%,rgba(2,9,18,.4) 100%)}.hero:after{display:none}.hero-text{background:rgba(2,10,20,.6);backdrop-filter:blur(14px);border:1px solid rgba(201,243,238,.15);border-radius:22px;padding:22px 18px}.hero-fish-banner{display:block;margin-top:18px;border-radius:20px;overflow:hidden;box-shadow:0 16px 40px rgba(0,0,0,.35)}.hero-fish-banner img{width:100%;display:block}.hero-inner{padding:40px 18px 60px;gap:30px}.hero h1{font-size:clamp(42px,13vw,60px)}.hero p{font-size:14.5px;margin-top:15px}.hero-ctas{margin-top:20px}.social-proof{margin-top:14px}.stats{margin-top:22px}.hero-cards{grid-template-columns:1fr 1fr;gap:9px}.hero-card{min-height:175px;padding:13px}.hero-card h3{font-size:15px;margin-top:12px}.hero-card p{font-size:11px}.hc-image{width:80px}.trust{grid-template-columns:1fr;padding:0 18px 25px}.section{padding:52px 18px}.section-head{align-items:flex-start;flex-direction:column}.product-grid{grid-template-columns:1fr 1fr;gap:11px}.product-art{height:170px}.product-body{padding:12px}.product-name{font-size:13px}.product-note{font-size:10.5px}.price{font-size:15px}.add{padding:7px 9px}.category-grid{grid-template-columns:1fr 1fr;gap:10px}.category-card{min-height:240px;padding:14px}.category-card h3{font-size:18px}.category-card p{font-size:10.5px}.care-grid{grid-template-columns:1fr}.story-grid{grid-template-columns:1fr;gap:25px}.story-visual{min-height:290px;order:-1}.story-points{gap:9px}.story-stats{flex-wrap:wrap;gap:10px 0}.story-stat{padding:0 16px 0 0;margin-right:16px}.story-stat strong{font-size:21px}.custom-box{padding:45px 18px;flex-direction:column;align-items:flex-start}.custom h2{font-size:32px}.search input{width:100%}.search{width:100%}.footer-inner{padding:35px 18px}.stats{gap:16px}.stat{margin-right:0;padding-right:16px}.hero-cards .hero-card:nth-child(n+3){min-height:160px}}
        ...
        .order-id{display:inline-block;background:#eef2ef;color:${C.navy};font-family:'Space Grotesk';font-weight:700;font-size:12.5px;padding:6px 14px;border-radius:999px;margin-bottom:16px;letter-spacing:.03em}
        .order-summary{text-align:left;background:#fff;border:1px solid ${C.line};border-radius:14px;padding:16px;margin-bottom:20px}
        .order-summary .os-item{display:flex;justify-content:space-between;font-size:12.5px;padding:7px 0;border-bottom:1px dashed ${C.line}}
        .order-summary .os-item:last-child{border-bottom:0}
        .order-summary .os-total{display:flex;justify-content:space-between;font-weight:800;font-size:13.5px;padding-top:10px;margin-top:4px;border-top:1px solid ${C.line}}
        .next-steps{text-align:left;margin-bottom:22px}
        .next-steps .ns-item{display:flex;gap:12px;align-items:flex-start;padding:10px 0}
        .next-steps .ns-num{flex:none;width:24px;height:24px;border-radius:50%;background:${C.navy};color:#fff;font-size:11.5px;font-weight:800;display:grid;place-items:center}
        .next-steps .ns-text{font-size:12.5px;color:#5a6d70;line-height:1.5;padding-top:2px}
        .next-steps .ns-text strong{color:${C.navy}}
      ...
      <main id="main-content">
        <section id="home" className="hero">
          ...
          <div className="hero-inner">
            <div className="hero-text">
              ...
              <a className="social-proof" href={GOOGLE_REVIEWS_URL} target="_blank" rel="noopener noreferrer"><div className="sp-stars">{[...Array(5)].map((_,i) => <Star key={i} size={15} fill="#f5b400" color="#f5b400"/>)}</div><span><strong><CountUp value={5} decimals={1}/></strong> rating · <strong><CountUp value={118} suffix="+"/></strong> happy customers on Google</span></a>
        ...
        <section id="story" className="section story">
          ...
          <div className="swim-fish"><Fish size={26}/></div>
          <Reveal className="story-grid"><div className="story-copy"><div className="eyebrow">Why Aqua Dreamland</div><h2 className="section-title">Small tanks. Big personality.</h2><p>We wanted aquarium shopping to feel less like picking a box of glass and more like building a little world. So we pair practical tank sizes with the small details that make a room feel yours.</p><div className="story-stats"><div className="story-stat"><strong><CountUp value={8} suffix="+"/></strong><span>Years in business</span></div><div className="story-stat"><strong><CountUp value={118} suffix="+"/></strong><span>Happy customers on Google</span></div><div className="story-stat"><strong><CountUp value={5} decimals={1}/></strong><span>Google rating</span></div></div><div className="story-points"><div className="point"><MapPin size={20}/><strong>Visit before you buy</strong><span>Come see real tanks and fish in person — not just photos online.</span></div><div className="point"><ShieldCheck size={20}/><strong>No middlemen</strong><span>Built in-house and sold direct — not resold at a markup.</span></div><div className="point"><Sparkles size={20}/><strong>Handpicked décor</strong><span>Pieces chosen to work around your tank and room.</span></div><div className="point"><Fish size={20}/><strong>Any size you need</strong><span>From a 1 ft starter to fully custom builds.</span></div><div className="point"><Heart size={20}/><strong>Made with care</strong><span>In-house builds with a personal-shop feel.</span></div><div className="point"><Truck size={20}/><strong>Careful handling</strong><span>Designed around safe packing and easy handover.</span></div></div></div><div className="story-visual"><div className="quote"><span className="quote-mark">“</span><strong>“Your tank should feel like part of your home.”</strong><span>Aqua Dreamland · Aquarium, décor & more</span></div></div></Reveal></section>

        <section className="section testimonials"><Reveal><div className="section-head centered"><div><a className="eyebrow pill" href={GOOGLE_REVIEWS_URL} target="_blank" rel="noopener noreferrer" style={{textDecoration:"none",cursor:"pointer"}}><Star size={11}/> 5.0 on Google · 118+ reviews</a><h2 className="section-title">What Customers Say</h2><p className="section-copy center">Real reviews from real customers, straight from our Google listing. <a href={GOOGLE_REVIEWS_URL} target="_blank" rel="noopener noreferrer" style={{color:"#137f8c",fontWeight:800}}>See all reviews on Google →</a></p></div></div><div className="testi-grid"><div className="testi-card"><div className="testi-stars">{[...Array(5)].map((_,i) => <Star key={i} size={14} fill="#f5b400" color="#f5b400"/>)}</div><p>“Very nice collection of fishes and aquariums. Beautiful and unique artefacts are also available. The owner is very sweet and amiable — overall a wonderful experience, must visit.”</p><span>— Google Review</span></div><div className="testi-card"><div className="testi-stars">{[...Array(5)].map((_,i) => <Star key={i} size={14} fill="#f5b400" color="#f5b400"/>)}</div><p>“This is the best place in Narnaul to buy fish and so many types of birds at a good price. If your pet is facing any problem, you can call them and they will help you.”</p><span>— Google Review</span></div><div className="testi-card"><div className="testi-stars">{[...Array(5)].map((_,i) => <Star key={i} size={14} fill="#f5b400" color="#f5b400"/>)}</div><p>“Best facility in the Narnaul area — Aqua Dreamland helped me achieve the best vibe in my office!”</p><span>— Google Review</span></div></div></Reveal></section>

        <section className="section gallery">
          <div className="bubbles">{[...Array(8)].map((_, i) => {const size = 6 + (i % 4) * 5; return <span key={i} className="bubble" style={{left: `${(i * 11 + 4) % 100}%`, width: size, height: size, animationDuration: `${10 + (i % 5) * 2.2}s`, animationDelay: `${-(i * 1.9)}s`}} />;})}</div>
          <div className="swim-fish"><Fish size={22}/></div>
          <Reveal>
      ...
      <div className="trust-band"><span><CreditCard size={17}/> 100% Prepaid Orders</span><span><RotateCcw size={17}/> Easy Returns</span><span><ShieldCheck size={17}/> Secure UPI Payment</span><div className="pay-badges"><img className="pay-logo" src="/payment-phonepe.png" alt="PhonePe"/><img className="pay-logo" src="/payment-googlepay.png" alt="Google Pay"/></div></div>
